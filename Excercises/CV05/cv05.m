...
D = Inf*ones(nr_states,N+1); D(1,1) = 0;
...
for i=1:N
    ...
    for ii = 1:nr_states
        ...
        m = length(idxs); D_temp = zeros(m,1);
        state_id = zeros(m,1);
        for iii = 1:m
        ...
        [minval, mipos] = min(D_temp);
        D(ii,i+1) = minval; 
        x_pred(ii,i) = state_id(mipos);
    end
...
cur_pos = minpos;
for i=N:-1:1
    cur_pos = x_pred(cur_pos,i);
    states_decoded{i} = states{cur_pos};
end

%dekodovani zpravy (soubor, etc... )
msg_decoded = '';
for i=2:N+1
...
%prevedly spravu na char
msg = ''; msg_start = 1;
for i=1:7
    cur_bin_char = msg_decoded(msg_start:msg_start+4);
    idxs = find(strcmp(decode_table(:,2),cur_bin_char));
